...
class WallpaperDownloader:
    ...
    def __download_bulk_images(self):
        for image in self.images:
            self.download_link = (
                f"https://w.wallhaven.cc/full/{image[0:2]}/wallhaven-{image}.jpg"
            )
            jpg_response = requests.get(self.download_link)
            if jpg_response.status_code == 404:
                self.download_link = (
                    f"https://w.wallhaven.cc/full/{image[0:2]}/wallhaven-{image}.png"
                )
                with open(f"{self.folder_name}/{image}.png", "wb") as f:
